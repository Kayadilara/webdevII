...
<body class="bg-gray-50">
    <header>
        ...
        <a href="/products" class="px-4">Verkoopactie</a>
        @guest
        <a class="px-1 py-1 border rounded float-right " href=" {{ route('login') }} ">log in</a>
        <a class="px-1 py-1 border rounded float-right " href=" {{ route('register') }} ">registreer</a>
    @endguest
    @auth
    <a class="px-1 py-1 border rounded float-right " href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Logout</a>
...
<form id="logout-form " action="{{ route('logout') }}" method="POST" style="display: none;">
    ...
    <form class="text-center rounded border-2 p-2 bg-yellow-200 fixed bottom-0 w-full" action="{{ route('newsletter') }}" method="post">
        ...
        {{ csrf_field() }}
        <button class="p-3 m-2 border rounded bg-yellow-50 w-1/5" type="submit" class="btn btn-primary">Meld je aan</button><br>
